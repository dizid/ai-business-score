  ...
  switch (card.id) {
    ...
    case 'consistently-beaten': {
      ...
      return topName
        ...
        : `AI knows this brand exists, but a competitor is usually mentioned first — beaten in ${n(p.beaten)} of ${n(p.completedCalls)} checks.`;
    }
    case 'leading':
    ...
    case 'top-rival': {
      ...
      return name
        ? `**${name}** is the competitor showing up most — mentioned in ${n(p.mentionCount)} of ${n(p.completedCalls)} checks. Worth understanding what makes them citable (content structure, third-party coverage, reviews).`
        : '';
    }
    default:
      return '';
...
export function buildScanReportMarkdown(payload: ValidatedPayload): string {
  ...
  push();
  push(`Based on ${payload.completedCalls} / ${payload.completedCalls + payload.failedCalls} successful checks.`);
  push();
