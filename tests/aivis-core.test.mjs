...
// New code prefers importing directly from shared/aivis/* submodules
// (see shared/CLAUDE.md's 2026-09-12 architecture-refactor entry) rather
// than adding a new export to aivis-core.mjs's backward-compat facade.
...
// Regression: a real production company ("Reuters") ended up with website
// "https://reuters com" (space instead of dot, most likely a mobile
// autocorrect typo) because nothing downstream of normalizeUrl() ever
// checked the result was a well-formed URL before it got persisted — the
// WHATWG URL parser itself considers a space a forbidden host code point,
// so this later made harmonia.mjs's `new URL(website)` throw at scan time.
// isValidWebsiteUrl() is the fix, wired into both enrich.mts and
// companies.mts's POST handler.
describe('isValidWebsiteUrl', () => {
  ...
  it('rejects the exact real-world typo that caused the production bug', () => {
